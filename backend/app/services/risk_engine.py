...
def assess_risk(content: str, source: str = "web") -> ScanResult:
    ...
    for finding in findings:
        ...
        for provider_result in provider_results:
            if provider_result.malicious:
                ...
                indicators.append(Indicator(type="reputation", description=f"{provider_result.provider} signale cette URL comme menace", weight=weight))
                score += weight
    ...
    threat_type = "phishing" if any(item.type in {"sensitive_data", "impersonation", "url", "reputation"} for item in indicators) else "suspicious" if indicators else "low risk"
    recommendations = ["Ne cliquez pas sur les liens suspects.", "Ne communiquez jamais votre OTP, PIN ou mot de passe.", "Vérifiez directement auprès du service officiel."] if indicators else ["Aucun indicateur majeur détecté. Restez néanmoins vigilant."]
    confidence = min(0.99, 0.45 + min(len(indicators), 5) * 0.1) if indicators else 0.25
    return ScanResult(score=score, level=level, threat_type=threat_type, indicators=indicators, recommendations=recommendations, urls=urls, confidence=confidence, score_breakdown=breakdown, threat_intelligence=intelligence, source=source)